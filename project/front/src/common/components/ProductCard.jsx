...
export default function ProductCard({ p }) {
  ...
  return (
    <div
      onClick={() => navigate(p.actionUrl || `/leasing/products/${p.id}`)}
    ...
    >
      ...
      <div className="self-stretch flex-1 px-3 flex flex-col justify-start items-start gap-1">
        <div className="self-stretch inline-flex justify-end items-start gap-2 mb-1">
          <div className="px-3 py-1 bg-white rounded-xl border border-[#080036] flex justify-center items-center">
            <div className="justify-start text-[#080036] text-sm font-normal font-['Figtree'] leading-normal">
              {p.productType || p.badgeLabel || "Location"}
            </div>
          </div>
        </div>
        <div className="text-left justify-start text-[#080036] text-base font-normal font-['Figtree'] truncate w-full">
          {p.postTitle}
        </div>
        <div className="text-left justify-start text-[#080036] text-base font-semibold font-['Figtree'] mt-auto">
          {p.price !== undefined ? p.price : p.pricePerMonth}
          {p.priceSuffix || "€/mois"}
        </div>
      </div>
    </div>
