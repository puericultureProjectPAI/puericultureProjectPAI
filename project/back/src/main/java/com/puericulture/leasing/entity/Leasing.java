...
import jakarta.persistence.MapsId; // 引入共享主键注解，用来让 Leasing.id 等于 Posts.id。
...
public class Leasing { // 定义 Leasing 类，对应类图中的 Leasing。

    @Id // 标记 id 为主键。
    private Long id; // 保存 Leasing 记录唯一编号，同时等于关联的 Posts.id。
    ...
    @MapsId // 让当前 Leasing.id 直接复用关联的 Posts.id。
    @OneToOne(fetch = FetchType.LAZY, optional = false) // 声明每条 Leasing 详情必须关联一条 Posts。
    @JoinColumn(name = "id", nullable = false) // 使用 id 作为外键列，连接 posts.id。
    private Posts post; // 保存对应的发布内容，表示这条 Post 的类型是 Leasing。
} // 结束 Leasing 实体类。
