...
import com.puericulture.troc.entity.Troc; // 引入 Troc 实体，它继承 Posts 并表示交换类别公告。
import com.puericulture.troc.mapper.TrocPostMapper; // 引入 mapper，用来把实体转换为响应 DTO。
import com.puericulture.troc.repository.TrocRepository; // 引入 TrocRepository，用来保存和查询交换公告。
...
import org.springframework.transaction.annotation.Transactional; // 引入事务注解，用来保证数据库操作一致。
...
public class TrocPostService { // 定义 TrocPostService，集中处理 US1 发布交换公告逻辑。
    ...
    private final TrocRepository trocRepository; // 保存 TrocRepository 依赖，用来操作 Troc 继承实体。

    private final UserRepository userRepository; // 保存 UserRepository 依赖，用来操作 users 表。

    public TrocPostService(TrocRepository trocRepository, UserRepository userRepository) { // 通过构造函数注入 repository 依赖。
        this.trocRepository = trocRepository; // 保存 trocRepository 到当前服务。
        this.userRepository = userRepository; // 保存 userRepository 到当前服务。
    } // 结束构造函数。

    @Transactional // 开启事务，保证创建 Troc 公告时 posts 表和 troc 表同时保存成功。
    public TrocPostResponse createTrocPost(CreateTrocPostRequest request) { // 创建一条 Troc 类型公告，对应 US1 的提交表单。
        ...
        Troc troc = new Troc(); // 创建 Troc 实体；由于 Troc extends Posts，所以它同时拥有公告公共字段。
        troc.setTitle(request.getTitle()); // 设置公告标题；该字段继承自 Posts。
        troc.setDescription(request.getDescription()); // 设置公告描述；该字段继承自 Posts。
        troc.setImagesReferences(request.getImagesReferences()); // 设置图片引用，当前版本只存字符串。
        troc.setOpen(true); // 新发布的公告默认开放给其他用户查看。
        troc.setAuthor(author); // 设置公告作者，匹配类图中的 Author 关系。
        troc.setEstimatedPrice(request.getEstimatedPrice()); // 设置交换物品估计价值，匹配 Troc.estimated price。
        Troc savedTroc = trocRepository.save(troc); // 直接保存 Troc，Hibernate 会根据 JOINED 继承同时写入 posts 和 troc 表。
        return TrocPostMapper.toResponse(savedTroc); // 把保存后的 Troc 实体转换成前端响应。
    ...
    public List<TrocPostResponse> findOpenTrocPosts() { // 查询开放中的 Troc 公告列表，对应列表展示验收标准。
        return trocRepository.findAllByOpenTrueOrderByIdDesc().stream() // 从 troc 继承实体读取开放公告并转成流。
                .map(TrocPostMapper::toResponse) // 把每条 Troc 实体转换成响应 DTO。
                .toList(); // 收集为不可变列表返回。
